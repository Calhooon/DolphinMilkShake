...
if [ "${1:-}" = "--worker" ]; then
  ...
  LOCKING="006a20${HASH}"
  RESULT=$(curl -sS --max-time 20 -X POST "${WALLET_URL}/createAction" \
    -H "Origin: ${WALLET_URL}" \
    -H 'Content-Type: application/json' \
    -d "{\"description\":\"dolphinsense provenance\",\"outputs\":[{\"lockingScript\":\"${LOCKING}\",\"satoshis\":0,\"outputDescription\":\"record proof\"}]}" 2>/dev/null)
